...
public class Staff extends Member{
	...
	public Staff(StaffInfo sInfo) {
		super(sInfo.id, sInfo.pw, sInfo.number, sInfo.age);
		this.department = sInfo.department;
		this.pList = new ArrayList<Program>();
		
		for(Program p : sInfo.pList)
			this.pList.add(p);
	}
	...
	public StaffInfo GetStaffInfo() {
		...
		sInfo.id = this.id;
		sInfo.pw = this.pw;
		sInfo.number = this.number;
		sInfo.age = this.age;
		sInfo.departement = this.department;
		sInfo.pList = new ArrayList<Prgram>();
	...
	public static boolean isSubmitDueValidCheck(String submitdue) {
		...
		if(Pattern.matches(pattern, submitdue)) {
			try {
					...
					if(submitdueYear == curYearR || submitdueYear == curYearR+1) {
